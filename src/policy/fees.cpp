...
void TxConfirmStats::removeTx(unsigned int entryHeight, unsigned int nBestSeenHeight, unsigned int bucketindex, bool inBlock)
{
    ...
    if (!inBlock && uint32_t(blocksAgo) >= scale) { // Only counts as a failure if not confirmed for entire period
        assert(scale != 0);
...
CFeeRate CBlockPolicyEstimator::estimateRawFee(int confTarget, double successThreshold, FeeEstimateHorizon horizon, EstimationResult* result) const
{
    ...
    if (confTarget <= 0 || uint32_t(confTarget) > stats->GetMaxConfirms())
        return CFeeRate(0);
...
CFeeRate CBlockPolicyEstimator::estimateSmartFee(int confTarget, FeeCalculation *feeCalc, bool conservative) const
{
    ...
    // Return failure if trying to analyze a target we're not tracking
    if (confTarget <= 0 || uint32_t(confTarget) > longStats->GetMaxConfirms()) {
    ...
    if (uint32_t(confTarget) > maxUsableEstimate) {
