...
bool AppInitMain(const CoreContext& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (!strMasterNodeBLSPrivKey.empty()) {
        ...
        fMasternodeMode = true;
        {
            ...
            ::activeMasternodeManager = std::make_unique<CActiveMasternodeManager>(keyOperator, *node.connman);
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                node.llmq_ctx.reset();
                node.llmq_ctx.reset(new LLMQContext(chainman.ActiveChainstate(), *node.connman, *node.evodb, *node.mnhf_manager, *node.sporkman, *node.mempool, node.mn_activeman, node.peerman, false, fReset || fReindexChainState));
    ...
    node.cj_ctx = std::make_unique<CJContext>(chainman.ActiveChainstate(), *node.connman, *node.mempool, node.mn_activeman,
