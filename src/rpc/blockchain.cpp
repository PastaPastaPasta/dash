...
std::tuple<std::unique_ptr<CCoinsViewCursor>, CCoinsStats, const CBlockIndex*, evo::CEvoSnapshot>
PrepareUTXOSnapshot(
    NodeContext& node,
...
UniValue WriteUTXOSnapshot(
    ...
    const evo::CEvoSnapshot& evo_snapshot,
    AutoFile& afile,
    const fs::path& path,
...
void ReconsiderBlock(ChainstateManager& chainman, uint256 block_hash, bool ignore_chainlocks = false) {
    {
        LOCK(chainman.GetMutex());
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ReconsiderBlock(chainman, hash, ignore_chainlocks);
...
{
    CConnman& m_connman;
    CMasternodeSync* const m_mn_sync;
    const bool m_was_active;
public:
    NetworkDisable(CConnman& connman, CMasternodeSync* mn_sync) : m_connman(connman), m_mn_sync(mn_sync), m_was_active(connman.GetNetworkActive()) {
        m_connman.SetNetworkActive(false, m_mn_sync);
        if (m_connman.GetNetworkActive()) {
            throw JSONRPCError(RPC_MISC_ERROR, "Network activity could not be suspended.");
        }
    };
    ~NetworkDisable() {
        m_connman.SetNetworkActive(m_was_active, m_mn_sync);
    };
};
...
static RPCHelpMan dumptxoutset()
{
    return RPCHelpMan{
        ...
        "This call may take several minutes. Make sure to use no RPC timeout (dash-cli -rpcclienttimeout=0)",
        {
            ...
            {"snapshot_type", RPCArg::Type::STR, RPCArg::Default(""), "The type of snapshot to create. Can be \"latest\" to create a snapshot of the current UTXO set or \"rollback\" to temporarily roll back the state of the node to a historical block before creating the snapshot of a historical UTXO set. This parameter can be omitted if a separate \"rollback\" named parameter is specified indicating the height or hash of a specific historical block. If \"rollback\" is specified and separate \"rollback\" named parameter is not specified, this will roll back to the latest valid snapshot block that currently be loaded with loadtxoutset."},
            {"rollback", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED,
                "Height or hash of the block to roll back to before creating the snapshot. Note: The further this value is from the tip, the longer this process will take. Consider setting a higher -rpcclienttimeout value in this case.",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const std::string snapshot_type{self.Arg<std::string>(1)};
    if (!request.params[2].isNull()) {
        ...
        target_index = ParseHashOrHeight(request.params[2], *node.chainman);
    ...
    if (target_index != tip) {
        ...
        if (node::fPruneMode) {
            ...
            for (const CBlockIndex* block = current_tip; block && block->nHeight > target_index->nHeight; block = block->pprev) {
                if ((block->nStatus & BLOCK_HAVE_MASK) != BLOCK_HAVE_MASK) {
        ...
        disable_network = std::make_unique<NetworkDisable>(connman, node.mn_sync.get());
    ...
    try {
    {
        ...
        if (target_index != chainstate->m_chain.Tip()) {
            ...
            std::tie(cursor, stats, tip, evo_snapshot) = PrepareUTXOSnapshot(node, *chainstate, node.rpc_interruption_point);
        }
    }

    if (error.isNull()) {
        result = WriteUTXOSnapshot(*chainstate, cursor.get(), &stats, tip, evo_snapshot, afile, path, temppath, node.rpc_interruption_point);
        ...
        if (invalidate_index) {
            try {
                ...
                LogPrintf("Failed to restore chain tip after dumptxoutset rollback error\n");
            }
        }
        throw;
    }
    ...
    const std::function<void()>& interruption_point)
{
    ...
    return {std::move(pcursor), *CHECK_NONFATAL(maybe_stats), tip, std::move(evo_snapshot)};
}
...
{
    auto [cursor, stats, tip, evo_snapshot]{WITH_LOCK(::cs_main, return PrepareUTXOSnapshot(node, chainstate, node.rpc_interruption_point))};
    return WriteUTXOSnapshot(chainstate, cursor.get(), &stats, tip, evo_snapshot, afile, path, tmppath, node.rpc_interruption_point);
}
