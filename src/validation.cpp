    ...
    do {
...

        {
            ...
            if (this == &m_chainman.ActiveChainstate() && pindexFork != pindexNewTip) {
                ...
                uiInterface.NotifyBlockTip(GetSynchronizationState(still_in_ibd), pindexNewTip);
            }
        }
...
void Chainstate::ResetBlockFailureFlags(CBlockIndex *pindex, bool ignore_chainlocks) {
    ...
    for (CBlockIndex* reconsidered : reconsidered_blocks) {
        if (!reconsidered->IsValid(BLOCK_VALID_TRANSACTIONS) || !reconsidered->HaveNumChainTxs()) continue;
...
try {
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(block, state, consensus_params)) {
            LogError("%s: *** found bad block at %d, hash=%s (%s)\n", __func__,
        ...
        if (nCheckLevel >= 2 && pindex) {
            ...
            if (!pindex->GetUndoPos().IsNull()) {
                if (!UndoReadFromDisk(undo, pindex)) {
                    LogError("VerifyDB(): *** found bad undo data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return false;
                }
            }
        }
        ...
        if (nCheckLevel >= 3) {
            if (curr_coins_usage <= chainstate.m_coinstip_cache_size_bytes) {
                ...
                if (res == DISCONNECT_FAILED) {
                    LogError("VerifyDB(): *** irrecoverable inconsistency in block data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return false;
                }
    ...
    if (pindexFailure) {
        LogError("VerifyDB(): *** coin database inconsistencies found (last %i blocks, %i good transactions before that)\n", chainstate.m_chain.Height() - pindexFailure->nHeight + 1, nGoodTransactions);
        return false;
    }
    ...
    std::string* error)
{
    ...
    std::optional<evo::EvoSnapshot> evo_snapshot;
    ...
    if (evo_marker != 0) {
        ...
        try {
            ...
            // EOF immediately after a completely decoded EvoSnapshot is required.
        }
    }
    ...
    }
    {
        ...
        if (evo_snapshot) {
            ...
            if (!evo::SeedEvoSnapshotState(*evo_snapshot, dmnman, qblockman, qsnapman,
                                           *helper.credit_pool_manager, *helper.ehf_manager,
                                           m_blockman, snapshot_start_block)) {
                return false;
            }
...
void Chainstate::RecordBackgroundMNListHash(const CBlockIndex* pindex, const CDeterministicMNList& mn_list)
{
    ...
    // Before DIP3 the special-tx processor leaves the update record default
    // constructed, so bind the canonical empty list to the block here, the
    // identity GetDeterministicMNListHash and the evo snapshot payload use.
    const uint256 mn_list_hash{mn_list.GetBlockHash().IsNull()
                                   ? evo::CanonicalMNListHash(CDeterministicMNList{block_hash, pindex->nHeight, 0})
                                   : evo::CanonicalMNListHash(mn_list)};
      ...
      std::function<void(bilingual_str)> shutdown_fnc)
{
    ...
    if (DeploymentActiveAt(index_new, GetConsensus(), Consensus::DEPLOYMENT_DIP0003)) {
        evo::EvoSnapshot retained_snapshot;
        CBlock base_block;
        std::string evo_error;
        ...
        if (!history_matches ||
            ...
            !evo::VerifyEvoSnapshotBaseBlock(retained_snapshot, base_block, evo_error)) {
