...
void Chainstate::RecordBackgroundMNListHash(const uint256& block_hash, const uint256& mn_list_hash)
{
    if (EvoDbIdentity() == EvoDbIdentity::NORMAL) {
        m_evoDb.WriteBackgroundMNListHash(block_hash, mn_list_hash);
        ...
        if (m_required_background_mn_list_hashes->contains(block_hash)) {
            m_evoDb.WriteBackgroundWorkMNListHash(block_hash, mn_list_hash);
        }
    }
}
...
void Chainstate::SetRequiredBackgroundMNListHashes(const std::vector<uint256>& block_hashes)
{
    ...
    m_required_background_mn_list_hashes.emplace(block_hashes.begin(), block_hashes.end());
}
    ...
    while (pindex != nullptr) {
        ...
        for (auto c : GetAll()) {
            ...
            if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) && (pindexFirstNeverProcessed == nullptr || pindex == snap_base)) {
                ...
                // non-null), or conflicts with a ChainLock
                // (pindexFirstConflicing is non-null), it is not required to be in
                // setBlockIndexCandidates.
