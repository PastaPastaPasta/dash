...
void CGovernanceManager::ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, CConnman& connman, bool enable_bip61)
{
    ...
    if (msg_type == NetMsgType::MNGOVERNANCESYNC) {
        ...
        if (!g_masternodeSync->IsSynced()) return;
    ...
    // A NEW GOVERNANCE OBJECT VOTE HAS ARRIVED
        ...
        if (ProcessVote(pfrom, vote, exception, connman)) {
            ...
            g_masternodeSync->BumpAssetLastTime("MNGOVERNANCEOBJECTVOTE");
            vote.Relay(connman);
        } else {
            LogPrint(BCLog::GOBJECT, "MNGOVERNANCEOBJECTVOTE -- Rejected vote, error = %s\n", exception.what());
            if ((exception.GetNodePenalty() != 0) && g_masternodeSync->IsSynced()) {
                LOCK(cs_main);
...
void CGovernanceManager::AddGovernanceObject(CGovernanceObject& govobj, CConnman& connman, const CNode* pfrom)
{
    ...
    g_masternodeSync->BumpAssetLastTime("CGovernanceManager::AddGovernanceObject");
...
{
    if (fDisableGovernance) return;
    if (g_masternodeSync == nullptr || !g_masternodeSync->IsSynced()) return;
...
bool CGovernanceManager::ConfirmInventoryRequest(const CInv& inv)
{
    ...
    if (!g_masternodeSync->IsBlockchainSynced()) return false;
...
bool CGovernanceManager::MasternodeRateCheck(const CGovernanceObject& govobj, bool fUpdateFailStatus, bool fForce, bool& fRateCheckBypassed)
{
    ...
    if (!g_masternodeSync->IsSynced() || !fRateChecksEnabled) {
        return true;
    }
