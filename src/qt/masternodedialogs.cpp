                                                                                          ...
                                                                                          QString& error) const
{
    ...
    if (m_type == MnType::Evo) {
        ...
        platform_node_id.SetHex(m_platform_node_id_edit->text().trimmed().toStdString());
...
void UpdateRegistrarDialog::validate()
{
    ...
    const bool ok{any_change && operator_valid && MasternodeWidgetUtil::isP2PKHAddress(m_voting_edit->text()) &&
