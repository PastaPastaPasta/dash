...
//! A "dash-shared-mn-sigs" envelope carrying an unsigned prepared dissolution,
//! exactly as one participant would paste it to another
QString MakeDissolveEnvelope(const uint256& pro_tx_hash)
{
    CMutableTransaction tx;
    tx.nVersion = 3;
...
void MasternodeMaintenanceTests::maintenanceEnvelopePreload()
{
    ...
    const QString envelope{MakeDissolveEnvelope(source->getProTxHash())};
    UniValue json;
    ...
    QString other_error;
    QVERIFY(!dialog.m_un_collector->importEnvelope(MakeDissolveEnvelope(uint256::ONE), other_error));
