...
bool SharedMnCreateDialog::validatePage(Page page, QString& err)
{
    ...
    case PageInvite:
        ...
        break;
    case PageApprovals:
        if (m_role == Role::Participant && !canSign()) {
...
void SharedMnCreateDialog::updateButtons()
{
    ...
    m_save_button->setEnabled(!m_busy);
...
void SharedMnCreateDialog::refreshAll()
{
    ...
    updateProgress();
    updateButtons();
    if (!m_dead_reason.isEmpty()) showError(m_dead_reason);
}
...
void SharedMnCreateDialog::reject()
{
    ...
    auto* save{box.addButton(tr("Save and Close"), QMessageBox::AcceptRole)};
