...
QWidget* DissolveDialog::buildNowTab()
{
    ...
    m_now_table->setHorizontalHeaderLabels({tr("Share"), tr("Refund address"), tr("Receives")});
    ...
    m_now_table->setTextElideMode(Qt::ElideNone);
    m_now_table->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents);
    m_now_table->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
    m_now_table->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
...
void DissolveDialog::updateNowPreview()
{
    ...
    for (size_t i = 0; i < m_shares.size(); ++i) {
        ...
        auto* share_item = new QTableWidgetItem(is_actor ? tr("%1 (you)").arg(label) : label);
        auto* refund_item = new QTableWidgetItem(ScriptAddress(m_shares[i].scriptRefund));
        auto* payout_item = new QTableWidgetItem(SharedMnFormatAmount(displayUnit(), preview.payouts[i]));
        if (is_actor) {
...
QWidget* DissolveDialog::buildStandbyTab()
{
    ...
    // been generated
    const auto build_view = [this, tab, layout](const QString& title, const QString& placeholder) {
