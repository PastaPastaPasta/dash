...
DBErrors WalletBatch::LoadWallet(CWallet* pwallet)
{
    ...
    for (auto desc_cache_pair : wss.m_descriptor_caches) {
        ...
        assert(spk_man);
        (static_cast<DescriptorScriptPubKeyMan*>(spk_man))->SetCache(desc_cache_pair.second);
    }

    // Set the descriptor keys
    for (auto desc_key_pair : wss.m_descriptor_keys) {
        auto spk_man = pwallet->GetScriptPubKeyMan(desc_key_pair.first.first);
        (static_cast<DescriptorScriptPubKeyMan*>(spk_man))->AddKey(desc_key_pair.first.second, desc_key_pair.second);
    }
    for (auto desc_key_pair : wss.m_descriptor_crypt_keys) {
        auto spk_man = pwallet->GetScriptPubKeyMan(desc_key_pair.first.first);
        (static_cast<DescriptorScriptPubKeyMan*>(spk_man))->AddCryptedKey(desc_key_pair.first.second, desc_key_pair.second.first, desc_key_pair.second.second);
    }
