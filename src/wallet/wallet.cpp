    ...
    for (auto pcoin : GetSpendableTXs()) {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (nCoinType == CoinType::ONLY_FULLY_MIXED) {
                ...
                found = pcoin->tx->vout[i].nValue == CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL || pcoin->tx->vout[i].nValue == CDeterministicMN::HIGH_PERFORMANCE_MASTERNODE_COLLATERAL;
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if (fMasternodeMode && (wtx.tx->vout[i].nValue == CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL || wtx.tx->vout[i].nValue == CDeterministicMN::HIGH_PERFORMANCE_MASTERNODE_COLLATERAL)) continue;
