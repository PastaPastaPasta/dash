...
BOOST_FIXTURE_TEST_CASE(recovery_payment_cursor_from_history, SeededWalletPair)
{
    ...
    // provides it to us.
    CPubKey their_xpub;
    uint256 their_chaincode;
    BOOST_REQUIRE(m_iface->getFriendshipXpub(ACCOUNT, m_their_id, m_my_id, their_xpub, their_chaincode));
    ...
    for (const uint32_t index : {0u, 3u}) {
        CTxDestination destination;
        BOOST_REQUIRE(m_iface->getFriendshipPaymentDestination(their_xpub, their_chaincode, index, destination));
    ...
    const auto derive = [&](uint32_t index) -> std::optional<CScript> {
        CTxDestination destination;
        if (!m_iface_b->getFriendshipPaymentDestination(their_xpub, their_chaincode, index, destination)) {
            return std::nullopt;
        }
