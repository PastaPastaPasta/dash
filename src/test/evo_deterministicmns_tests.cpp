...
static CMutableTransaction CreateProRegTx(const CTxMemPool& mempool, SimpleUTXOMap& utxos, int port, const CScript& scriptPayout, const CKey& coinbaseKey, CKey& ownerKeyRet, CBLSSecretKey& operatorKeyRet)
{
    ...
    FundTransaction(tx, utxos, scriptPayout, CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL, coinbaseKey);
...
void FuncTestMempoolReorg(TestChainSetup& setup)
{
    ...
    FundTransaction(tx_collateral, utxos, scriptCollateral, CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL, setup.coinbaseKey);
    ...
    for (size_t i = 0; i < tx_collateral.vout.size(); ++i) {
        if (tx_collateral.vout[i].nValue == CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL) {
    ...
    FundTransaction(tx_reg, utxos, scriptPayout, CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL, setup.coinbaseKey);
...
void FuncTestMempoolDualProregtx(TestChainSetup& setup)
{
    ...
    for (size_t i = 0; i < tx_reg1.vout.size(); ++i) {
        if (tx_reg1.vout[i].nValue == CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL) {
    ...
    FundTransaction(tx_reg2, utxos, scriptPayout, CDeterministicMN::REGULAR_MASTERNODE_COLLATERAL, setup.coinbaseKey);
