...
void DashTestSetup(NodeContext& node, const CChainParams& chainparams)
{
    ...
    node.cj_ctx = std::make_unique<CJContext>(chainstate, *node.connman, *node.mempool, /* mn_activeman = */ nullptr, *node.mn_sync, /* relay_txes */ true);
...
#endif // ENABLE_WALLET
    node.llmq_ctx = std::make_unique<LLMQContext>(chainstate, *node.connman, *node.evodb, *node.mnhf_manager, *node.sporkman, *node.mempool, /* mn_activeman = */ nullptr, node.peerman, true, false);
