...
template<class T>
void TestTxHelper(const CMutableTransaction& tx, ProtxBLSSchemeRules bls_rules, bool expected_failure, const std::string& expected_error)
{
    ...
    BOOST_CHECK_EQUAL(opt_payload->IsTriviallyValid(bls_rules, dummy_state), !expected_failure);
    if (expected_failure) {
...
void trivialvalidation_runner(const std::string& json)
{
    ...
    for (size_t idx = 1; idx < vectors.size(); idx++) {
        ...
        try {
            ...
            ProtxBLSSchemeRules bls_rules{test[2].get_str() == "basic", false};
            // Raw transaction
            ...
            case TRANSACTION_PROVIDER_REGISTER: {
                ...
                TestTxHelper<CProRegTx>(tx, bls_rules, expected, expected_err);
                break;
            }
            case TRANSACTION_PROVIDER_UPDATE_SERVICE: {
                BOOST_CHECK_EQUAL(txType, "proupservtx");

                TestTxHelper<CProUpServTx>(tx, bls_rules, expected, expected_err);
                break;
            }
            case TRANSACTION_PROVIDER_UPDATE_REGISTRAR: {
                BOOST_CHECK_EQUAL(txType, "proupregtx");

                TestTxHelper<CProUpRegTx>(tx, bls_rules, expected, expected_err);
                break;
            }
            case TRANSACTION_PROVIDER_UPDATE_REVOKE: {
                BOOST_CHECK_EQUAL(txType, "prouprevtx");

                TestTxHelper<CProUpRevTx>(tx, bls_rules, expected, expected_err);
                break;
            }
            default:
