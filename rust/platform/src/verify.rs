...
//! Drive proved-response verification for the DAPI queries the dash-qt
//! Platform GUI makes, built on drive-proof-verifier's `FromProof`: each
//! function takes the exact protobuf request the transport sent plus the
//! full protobuf response it received, reconstructs the query from the
//! request, replays the GroveDB proof, and verifies the Tenderdash BLS
//! quorum threshold signature against the keys served by [`crate::provider`].
