...
//! queries. dash-qt serves both from local knowledge — quorum keys are
//! pushed across the bridge from the node's synced LLMQ data
//! (PlatformClient::updateQuorumKeys), and the only contracts the GUI ever
//! queries are the pinned DPNS and DashPay system contracts compiled into
//! dpp — so proof verification never performs network fetches.
//!
//! The provider is process-global: dash-qt runs one Platform client against
...
/// Dash Core's internal uint256 order; the C++ adapter converts).
...
struct State {
    ...
    /// Core height at which Platform activated (mn_rr). Only consulted by
    /// FromProof paths the GUI does not use (getTotalCreditsInPlatform);
    /// dash-qt currently pushes 0.
...
pub struct CoreContextProvider {
    state: RwLock<State>,
}

static PROVIDER: OnceLock<CoreContextProvider> = OnceLock::new();

/// The process-global provider handed to every FromProof call.
pub fn provider() -> &'static CoreContextProvider {
    PROVIDER.get_or_init(|| CoreContextProvider {
...
/// Parses a Dash Core network id ("main", "test", "regtest", "devnet") into
...
impl ContextProvider for CoreContextProvider {
    ...
    fn get_token_configuration(
        ...
        Err(ContextProviderError::Generic(
            "token configurations are not available in dash-qt".to_string(),
        ))
    }
