...
//! come from the fixture corpus in src/test/data/platform — push the
...
fn quorum_valid() -> QuorumVector {
    let file: QuorumVectorFile = serde_json::from_str(include_str!(
...
fn setup() {
    ...
    ONCE.call_once(|| {
        dash_platform_ffi::provider::set_context("test", 0).expect("set_context");
        let quorum = quorum_valid();
        dash_platform_ffi::provider::update_quorum_keys(
            quorum.envelope.quorum_type as u8,
            vec![dash_platform_ffi::provider::QuorumKey {
...
fn check_meta(meta: &dash_platform_ffi::types::Meta) {
    let ctx = quorum_valid().block_context;
...
// --- request/response synthesis (mirrors src/platform/transport/client.cpp)
...
fn identity_contract_nonce_verifies_end_to_end() {
    ...
    let (nonce, meta) =
        dash_platform_ffi::verify::verify_get_identity_contract_nonce(&request, &response)
...
fn identity_balance_verifies_end_to_end() {
    ...
    let (balance, meta) =
        dash_platform_ffi::verify::verify_get_identity_balance(&request, &response)
...
fn identity_keys_verify_end_to_end() {
    ...
    let (keys, meta) = dash_platform_ffi::verify::verify_get_identity_keys(&request, &response)
...
fn contested_vote_state_active_verifies_end_to_end() {
    ...
    let (state, meta) =
        dash_platform_ffi::verify::verify_get_contested_vote_state(&request, &response)
...
fn unknown_quorum_is_rejected() {
    ...
    let request = identity_nonce_request(hexv(&file.identity_id));
    let response = identity_nonce_response(proof);
    let err = dash_platform_ffi::verify::verify_get_identity_nonce(&request, &response)
